<?php
/**
 * This file is part of the Roulette package.
 *
 * (c) Eko Dedy Purnomo <eko.dedy.purnomo@gmail.com>
 *
 * For the full copyright and license information, please view the LICENSE
 * file that was distributed with this source code.
 */
namespace Roulette\Mixin;

/**
 * ##Configurable
 * Easy to get set or reconfigure each properties even it is `private`.
 * Properties name started with `_` (underscore) mean unconfigurable/inaccesible
 *      
 *      class Person extends \Roulette\Base{
 *          protected $name = null; //configurable property
 *          protected $_tall = null; //unconfigurable property
 * 
 *          function __construct($config = null){
 *              parent::__construct($config);
 *          }
 *      }
 *      
 *      // easy to construct object using array to pass to the matched properties
 *      $person = new Person(array(
 *          'name'=>'Eko',
 *          '_tall'=>170
 *      ));
 * 
 *      // easy to set and get properties value
 *      $person->getConfig('name'); // return 'Eko'
 *      $person->setConfig('name','Dedy');
 *      $person->getConfig('name'); // return 'Dedy'
 *  
 *      // easy to protect property using `_` in the first variable name 
 *      $person->getConfig('_tall'); // return null
 *      $person->setConfig('_tall','168');
 *      $person->getConfig('_tall'); // still return null 
 *
 * @package Roulette\Mixin
 * @since Version 2.0.0
 * @author Eko Dedy Purnomo <eko.dedy.purnomo@gmail.com>
 */
trait Configurable
{
    /**
     * Merge its config with parent config.
     * this will merge the parent config into it from parent config,
     * this method better to use in merge some config within value is array
     *
     * @param array $config a set of configuration to be applied to the class
     * @return \Roulette\Base 
     */
    protected function mergeParentConfig( $config = null )
    {
        if ( empty($config) || !property_exists($this, $config) ) return;
        
        $parentClass = get_parent_class($this);
        if ( $parentClass )
        {
            $parent = new $parentClass; // prototyping parent object
            $this->$config = array_merge($parent->$config, $this->$config);
        }
        return $this->$config;
    }

    /**
     * Configure class using passed arguments (Array necessaries)
     * 
     *      Example:
     *      class Person extends \Roulette\Base
     *      {
     *          protected $name = null;
     *          function __construct($config){
     *              function __construct($config)
     *          }
     *      }
     *      $person->configure(array(
     *          'name'=>'Eko'
     *      ));
     *      $person->getConfig('name'); // return 'Eko'
     *      
     * @param Array $config New configs to be applied in
     * @return \Roulette\Base 
     */
    protected function configure(Array $config = null, $options = null)
    {   
        $only = (is_array($options) and array_key_exists('only', $options)) ? $options['only'] : null;
        $except = (is_array($options) and array_key_exists('except', $options)) ? $options['except'] : null;
    
        if ( is_array($config) )
        {
            foreach ($config as $configName => $configValue)
            {
                # filter for condition
                if ( is_array($only) and !in_array($configName, $only) ) { continue; }
                if ( is_array($except) and in_array($configName, $except) ) { continue; }

                $this->setConfig($configName, $configValue, true);
            }
        }

        return $this;
    }

    /**
     * Sorthand for getConfig if pass one argument. 
     * Sorthand for setConfig if pass two or more arguments.
     * 
     * @return Mixed Value depend on the arguments passed 
     */
    protected function config()
    {
        $args_num = func_num_args();
        if ($args_num <= 1) {
            return $this->getConfig(func_get_arg(0));
        } else {
            return $this->setConfig(func_get_arg(0), func_get_arg(1));
        }
    }

    /**
     * Get value from a config/property
     * @param string $configName Config/property name
     * @param mixed $defaultValue
     * @return Mixed Config/property value
     */
    protected function getConfig($configName = null, $defaultValue = null)
    {
        # doesn't allow any property started with `_`
        if (preg_match("/^(_+)/", $configName, $matches))
            return;

        if (property_exists($this, $configName))
        {
            return $this->$configName;
        }
        else
        {
            return $defaultValue;
        }
    }

    /**
     * Set value for a config/property
     * 
     * @param string $configName Config/property name
     * @param Mixed $configValue Value to be set into the property
     * @param boolean $merge_config merge config with its existing config, used if existing value is array
     * @return \Roulette\Base
     */
    protected function setConfig( $configName = null, $configValue = null, $merge_config = false )
    {
        if (preg_match("/^(_+)/", $configName, $matches))
            return $this;

        if ( is_array($configValue) 
            and property_exists($this, $configName) 
            and is_array($this->$configName) 
            and $merge_config === true )
        {
            $this->$configName = array_merge($this->$configName, $configValue);
        } 
        else 
        {
            $this->$configName = $configValue;
        }
        return $this;
    }

    /**
     * Set config or property to `true`
     * 
     * @param string $configName Config/Property name to be setted
     * @param boolean $enabled Enabled status
     * @return \Roulette\Base
     */
    protected function enableConfig($configName = null, $enabled = true){
        return $this->setConfig($configName, (boolean) $enabled);
    }

    /**
     * Check if config has `true` value
     * 
     * @param string $configName Config name
     * @return boolean Enabled status
     */
    protected function configEnabled($configName = null){
        return (boolean) $this->getConfig($configName);
    }

    /**
     * Set config or property to `true`
     * 
     * @param string $configName Config/Property name to be setted
     * @param boolean $disabled Enabled status
     * @return \Roulette\Base
     */
    protected function disableConfig($configName = null, $disabled = true){
        return $this->setConfig($configName, ! (boolean) $disabled);
    }

    /**
     * Check if config has `true` value
     * 
     * @param string $configName Config name
     * @return boolean Enabled status
     */
    protected function configDisabled($configName = null){
        return ! (boolean) $this->getConfig($configName);
    }

    /**
     * Check existense of config name.
     * @param  string  $configName Config name
     * @return boolean
     */
    protected function hasConfig($configName = null){
        return property_exists($this, $configName);
    }
}