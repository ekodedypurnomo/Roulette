 ...
 */
class Phalcon3 extends TunelAbstract
{
	...
	protected function captureLog(callable $process, callable $callback)
	{
		...
		// do the process
		call_user_func_array($process, array($this));
		...
		// callback
		call_user_func_array($callback, array($queryString, $newLog));

		return $this;
	}

	protected function operateQuery(Operation $operation)
	{
		$configs = Collection::create($operation->getOption())->setIfNot(array(
			'query' => null,
			'result_array' => false
		));
	...
	protected function operateSelect(Operation $operation)
	{
	...
	protected function buildWhere(array $where, $builder)
	{
		$me = $this;
		...
		foreach ($where as $i => $condition) 
		{
			...
			# basic
			else if ( in_array($_operator, explode('|', '=|<|>|<>|<=|>=')) )
			{
				...
				else if (preg_match('/^({column}|{date}|{time}|{day}|{month}|{year})(.*)/', $value, $matches)) 
				{
					$type = $matches[1];
			...
			# between
			else if (in_array($_operator, array('BETWEEN', 'NOT BETWEEN')))
			{
				$builder->whereBetween($field, $value, $hook, preg_match('/^NOT/', $_operator) );
			}

			# inclusion
			else if (in_array($_operator, array('IN', 'NOT IN')))
			{
				$builder->whereIn($field, $value, $hook, preg_match('/^NOT/', $_operator) );
			}

			# null
			else if (in_array($_operator, array('NULL', 'NOT NULL')))
			{
	...
	protected function buildSelect($select, $builder)
	{
		if (is_array($select))
		{
	...
	protected function buildGroup(array $group, $builder)
	{
		$builder->groupBy($group);
	}

	protected function buildHaving(array $having, $builder)
	{
		$builder->havingRaw($having);
	}

	protected function buildOrder($order, $builder)
	{
		if ( is_array($orders) )
		{
	...
	protected function buildLimit($limit, $builder)
	{
		$builder->take($limit);
	}

	protected function buildOffset($offset, $builder)
	{
		$builder->skip($offset);
	}

	protected function operateExist( Operation $operation )
	{
		$configs = Collection::create($operation->getOption())->setIfNot(array(
			'table'		=> null,
			'condition'	=> array()
	...
	protected function operateInsert( Operation $operation )
	{
		$option = $operation->getOption();
	...
	protected function operateUpdate( Operation $operation )
	{
		$option = $operation->getOption();
	...
	protected function operateDelete( Operation $operation )
	{
		$option = $operation->getOption();
	...
	protected function operateTruncate( Operation $operation )
	{
		$option = $operation->getOption();
